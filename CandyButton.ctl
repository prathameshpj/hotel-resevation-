...
Private Enum eMsgWhen                                                       'When to callback
    MSG_BEFORE = 1
                                                            'Callback before the original WndProc
    MSG_AFTER = 2
                                                             'Callback after the original WndProc
    MSG_BEFORE_AFTER = MSG_BEFORE Or MSG_AFTER
                                'Callback before and after the original WndProc
End Enum
#If False Then 'Trick preserves Case of Enums when typing in IDE
Private MSG_BEFORE, MSG_AFTER, MSG_BEFORE_AFTER
#End If
Private Const ALL_MESSAGES               As Long = -1     'All messages callback
Private Const MSG_ENTRIES                As Long = 32     'Number of msg table entries
Private Const WNDPROC_OFF                As Long = &H38
 'Thunk offset to the WndProc execution address
...
Private Const IDX_PARM_USER              As Long = 13
     'Thunk data index of the User-defined callback parameter data index
...
End Enum
#If False Then 'Trick preserves Case of Enums when typing in IDE
Private PIC_TOP, PIC_BOTTOM, PIC_LEFT, PIC_RIGHT
#End If
Public Enum eStyle
    XP_Button
    XP_ToolBarButton
...
End Enum
#If False Then 'Trick preserves Case of Enums when typing in IDE
Private XP_Button, XP_ToolBarButton, Crystal, Mac, Mac_Variation, WMP, Plastic, Iceblock
#End If
...
End Enum
#If False Then 'Trick preserves Case of Enums when typing in IDE
Private Custom, Aqua, WMP10, DeepBlue, DeepRed, DeepGreen, DeepYellow
#End If
Public Enum eState
    eNormal
    ePressed
...
End Enum
#If False Then 'Trick preserves Case of Enums when typing in IDE
Private eNormal, ePressed, eFocus, eHover, eChecked
#End If
...
Private Declare Function TranslateColor Lib "olepro32.dll" Alias "OleTranslateColor" (ByVal clr As OLE_COLOR, _
...
Dim YCoord As Long
    ...
    End If
    If LenB(m_Caption) Then
        Call DrawStateText(hdc, 0&, 0&, m_Caption, Len(m_Caption), XCoord, YCoord, 0&, 0&, lFlags)
    End If
...
'z_Funk.Remove "h" & lng_hWnd                                            'Remove the specified window handle from the collection
    End If
End Sub
Private Function SetBound(ByRef Num As Long, _
                          ByRef MinNum As Long, _
                          ByRef MaxNum As Long) As Long
    If Num < MinNum Then
        SetBound = MinNum
...
Private Sub UserControl_EnterFocus()
    ...
    If Not m_bEnabled Then
        Exit Sub
    End If
    If Not m_Checked And Not IsHover Then
        DrawButton (eFocus)
    End If
End Sub
...
Private Sub UserControl_MouseMove(Button As Integer, _
    ...
    If Button = 1 And (X < 0 Or X > ScaleWidth Or Y < 0 Or Y > ScaleHeight) Then
        IsHover = False
        DrawButton (eNormal)
    End If
End Sub
...
Dim i      As Long   'Loop index
    nBase = z_ScMem
                                                            'Remember z_ScMem so that we can restore its value on exit
    z_ScMem = zData(nTable)
                                                    'Map zData() to the specified table
    ...
    Else
        ...
        For i = 1 To nCount
            ...
            End If
        Next i
                                                                  'Next message table entry
        nCount = i
                                                              'On drop through: i = nCount + 1, the new table entry count
        zData(nCount) = uMsg
                                                    'Store the message in the appended table entry
    End If
    zData(0) = nCount
                                                         'Store the new table entry count
Bail:
    z_ScMem = nBase
                                                           'Restore the value of z_ScMem
End Sub
    ...
    Else
        ...
        For i = 1 To nCount
            ...
            If zData(i) = uMsg Then
                ...
                zData(i) = 0
                                                        'Null the msg value -- also frees the element for re-use
                GoTo Bail                                                           'Bail
            End If
        Next i
                                                                  'Next message table entry
...
Private Function zMap_hWnd(ByVal lng_hWnd As Long) As Long
    ...
    End If
Exit Function
                                                             'Exit returning the thunk address
Catch:
